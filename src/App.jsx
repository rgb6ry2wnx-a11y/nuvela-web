...
export default function App() {
  ...
  // ==== Agenda tu Cita ====
  // Single step: one card per advisor, each linking out to THEIR OWN Google
  // Calendar "Appointment Schedule" page (hours Mon–Sat 9am–5pm, 1-hour
  // slots) — every advisor sets one up once inside their own Google
  // Calendar and gives Eduardo the booking-page link, pasted into the
  // `url` field below. Those pages refuse to be embedded in an <iframe>
  // (Google sends X-Frame-Options), so instead of embedding them we show a
  // button that opens the right one in a new tab. Real availability + real
  // bookings are handled entirely by Google — no backend needed on our
  // side. An advisor whose `url` is still empty falls back to a WhatsApp
  ...
  const CITA_ADVISORS = [
    { name: 'Manuel M.', role: { es: 'Ventas', en: 'Sales' }, url: 'https://calendar.app.google/b8xtQpJnex2KvAyT8', whatsapp: '50235465165' },
  ...
  // second location later is just one more entry — just give it its own
  // `address` (business name + street address, exactly as it appears on
  // Google Maps) and everything else follows automatically.
  //
  // IMPORTANT: the embed URL is built from `address` (plain text), NOT
  // from a Google "place_id". The key-free embed endpoint only knows how
  // to geocode normal address text — feeding it a place_id silently fails
  // and centers the map on (0,0), off the coast of Africa. Address text is
  // what actually shows the real pin in Guatemala. Keep this in sync with
  // the equivalent block in index.html (SUCURSALES / renderCitaSucursales()).
  const SUCURSALES = [
    {
      ...
      address: 'Nuvela, 1A Avenida 8-74, Zona 9, Ciudad de Guatemala 01009, Guatemala',
    },
  ];
        ...
        <main className="page-fade">
          <section className="bg-cream pt-28 pb-16">
            <div className="max-w-4xl mx-auto px-6 text-center reveal">
              ...
              <p className="text-graphite mt-6 leading-relaxed max-w-xl mx-auto">
                {t('Elige a tu asesor y agenda tu horario con un clic — Lunes a Sábado, de 9:00am a 5:00pm, citas de una hora.', 'Choose your advisor and book your time in one click — Monday to Saturday, 9:00am to 5:00pm, one-hour appointments.')}
              </p>
            </div>
          </section>

          {/* Choose an advisor -> their own live Google Calendar */}
          ...
          <section className="pb-24 md:pb-32 bg-white border-t border-pearl">
            <div className="max-w-4xl mx-auto px-6 pt-16">
              ...
              <div className="mt-8 grid gap-8">
                {SUCURSALES.map((s) => {
                  const embedSrc = `https://www.google.com/maps?q=${encodeURIComponent(s.address)}&output=embed`;
