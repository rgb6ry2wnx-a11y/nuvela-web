...
// Línea para Hoteles: mattresses, pillows, bed accessories (duvets/protectors) and loungers.
...
// demás (ej. "Ninguna de estas"). Mantén esto igual al bloque equivalente
// en index.html (QUIZ_CATEGORY_QUESTION / QUIZ_QUESTIONS / QUIZ_CANDIDATES /
// RESULT_BLURBS) para que ambas versiones del sitio den el mismo resultado.
...
export default function App() {
  ...
  const visibleProducts = activeCategory === null ? [] : (activeCategory === 'all' ? PRODUCTS : PRODUCTS.filter((p) => p.category.es === activeCategory));
  const selectCategory = (key) => {
    setActiveCategory(key);
    setTimeout(() => {
      if (productsGridRef.current) productsGridRef.current.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 60);
  };
  ...
  const cartHasCategory = (categoryEs) =>
    cart.some((e) => {
      const line = cartLine(e);
  ...
  // Total final después del descuento (si corresponde) — el que se muestra,
  // se manda por WhatsApp y se usa para calcular cuotas.
  const finalCartTotal = () => Math.max(0, cartTotal() - discountAmount());

  const applyDiscountCode = () => {
    const raw = discountInput.trim().toUpperCase();
    if (!raw) return;
    setAppliedDiscountCode(raw); // DISCOUNT_CODES lookup en el render decide si es real
  };
  ...
  const [quoteForm, setQuoteForm] = useState({ nombre: '', telefono: '', empresa: '', correo: '', comentarios: '' });
  ...
  const requestQuoteViaWhatsApp = () => {
    ...
    if (missing) {
      setQuoteError(t('Completa nombre, teléfono, hotel/empresa y correo para continuar.', 'Please fill in name, phone, hotel/company and email to continue.'));
      return;
    }
    setQuoteError('');
  ...
  // added inside Google Calendar's settings), so CITA_ADVISORS here is just
  // "meet the team" — plain cards with no individual booking link — and
  // there's ONE button underneath that opens the shared calendar. Earlier
  // this showed a separate booking button per advisor (each their own
  // calendar), which double-booked the one room; before that it was fixed
  // by pointing all 3 buttons to the same calendar, but that still asked
  // "which advisor" twice (card, then Google's form) — confusing — so as of
  // Sept 2026 there's only one prompt, inside Google. If
  ...
  // Each branch renders as a card with an embedded Google Maps iframe (no
  // API key needed — the classic "output=embed" trick) plus a "Cómo
  // llegar" button that opens real Google Maps directions in a new tab.
  // Today there's only one branch (Zona 9), but this is a list so adding a
  // second location later is just one more entry.
  //
  ...
  //    real one, and a raw "place_id:" is not understood by this key-free
  //    endpoint at all — it silently centers on (0,0), off the coast of
  //    Africa. Coordinates are unambiguous.
  ...
  // To find a new branch's lat/lng + placeId: open its Google Maps listing
  // in a browser, and read them out of the address bar URL
  // (.../place/Name/@LAT,LNG,17z/.../1s<PLACE_ID>...). Keep this in sync
  // with the equivalent block in index.html (SUCURSALES / renderCitaSucursales()).
  const SUCURSALES = [
    {
  ...
  // Beneficios/Especificaciones: plegados por defecto en móvil (el CSS las
  // fuerza siempre abiertas en pantallas grandes, sin importar este estado).
  const [pdBenefitsOpen, setPdBenefitsOpen] = useState(false);
  const [pdSpecsOpen, setPdSpecsOpen] = useState(false);
  ...
  // "+ Agregar a Cotización" feedback for the Línea para Hoteles cards.
  ...
  // Al agregar un producto a la cotización, llevamos el scroll hasta esa
  // línea (arriba de la página) para que el cliente vea de inmediato que se
  // agregó — luego puede volver a bajar y seguir agregando más productos.
  const quoteRowDomId = (productId, variantName) => `quote-row-${encodeURIComponent(`${productId}::${variantName || ''}`)}`;
  const [quoteRowHighlight, setQuoteRowHighlight] = useState({});
  const scrollToQuoteRow = (productId, variantName) => {
    const rowKey = `${productId}::${variantName || ''}`;
    setTimeout(() => {
      const el = document.getElementById(quoteRowDomId(productId, variantName));
      if (el) {
        el.scrollIntoView({ behavior: 'smooth', block: 'center' });
        setQuoteRowHighlight((prev) => ({ ...prev, [rowKey]: true }));
        setTimeout(() => setQuoteRowHighlight((prev) => ({ ...prev, [rowKey]: false })), 1400);
      }
    }, 50);
  };

  // Shared product card markup — used by both the main catalog and Línea para Hoteles.
  ...
  const ProductCard = (p, opts) => {
    ...
    return (
      <div key={p.id} className="bg-white border border-pearl reveal flex flex-col overflow-hidden">
        <div className="aspect-[4/3] sm:aspect-square overflow-hidden bg-cream img-fallback">
        ...
        <div className="p-3 sm:p-8 flex flex-col flex-grow">
          <Eyebrow className="!text-[0.6rem] sm:!text-[0.72rem] !tracking-[0.16em] sm:!tracking-[0.32em]">{pick(p.eyebrow)}</Eyebrow>
          <h3 className="font-serif text-base sm:text-2xl text-ink mt-1 sm:mt-3 leading-tight">{pick(p.name)}</h3>
          <p className="hidden sm:block text-graphite text-sm mt-3 leading-relaxed flex-grow">{pick(p.cardText)}</p>
          {opts.hideRetailPrice ? (
            <>
              <p className="hidden sm:block text-graphite text-sm mt-6 leading-relaxed">{t('Precios especiales por volumen para proyectos de hospitalidad.', 'Special volume pricing for hospitality projects.')}</p>
              {p.variants && p.variants.length > 1 && (
                <select
                  className="mt-2 sm:mt-4 border border-pearl px-2 sm:px-3 py-1.5 sm:py-2.5 text-xs sm:text-sm bg-white w-full"
              ...
              <button
                onClick={() => { addToQuote(p.id, quoteVariant, 1); flashQuoteAdded(p.id); scrollToQuoteRow(p.id, quoteVariant); }}
                disabled={!!quoteAddedFeedback[p.id]}
                className="btn-gold mt-2 sm:mt-4 w-full !text-[0.65rem] sm:!text-xs !py-2 sm:!py-3"
              >
              ...
              <button
                ...
                className="btn-outline mt-2 sm:mt-3 w-full !text-[0.65rem] sm:!text-xs !py-2 sm:!py-3"
              >
                {t('Ver mi Cotización', 'View My Quote')}
              </button>
            </>
          ) : (
            <>
              <p className="text-mist text-[0.6rem] sm:text-xs tracking-[0.18em] uppercase mt-3 sm:mt-6">{t('Desde', 'From')}</p>
              <p className="font-price text-xl sm:text-3xl text-gold mt-1">{from ? `Q${from.toLocaleString('en-US')}` : t('Próximamente', 'Coming soon')}</p>
              <button onClick={() => openProduct(p.id)} className="btn-outline mt-3 sm:mt-6 w-full !text-[0.65rem] sm:!text-xs !py-2 sm:!py-3">{t('Ver Detalle', 'View Details')}</button>
            </>
          )}
        </div>
      </div>
...
/* navbar scroll shadow */
  ...
  const openProduct = (id) => {
    ...
    setPdQty(1);
    setPdBenefitsOpen(false);
    setPdSpecsOpen(false);
    go('producto-detalle');
  };

  // ==== Examen de Productos / Product Quiz ====
  const [quizOpen, setQuizOpen] = useState(false);
  const [quizPhase, setQuizPhase] = useState('category'); // category | questions | reveal | result
  const [quizCategory, setQuizCategory] = useState(null);
  const [quizQIndex, setQuizQIndex] = useState(0);
  const [quizAnswers, setQuizAnswers] = useState({});
  const [quizResult, setQuizResult] = useState(null);

  const quizQuestions = () => (quizCategory ? (QUIZ_QUESTIONS[quizCategory] || []) : []);
  const quizCurrentQuestion = () => {
    if (quizPhase === 'category') return QUIZ_CATEGORY_QUESTION;
    if (quizPhase === 'questions') return quizQuestions()[quizQIndex];
    return null;
  };
  const quizProgressFraction = () => {
    const qs = quizQuestions();
    const total = 1 + qs.length + 1; // categoría + preguntas + reveal/resultado
    let current = 0;
    if (quizPhase === 'questions') current = 1 + quizQIndex;
    else if (quizPhase === 'reveal' || quizPhase === 'result') current = 1 + qs.length;
    return Math.round((current / total) * 100);
  };

  const openQuiz = () => {
    setQuizOpen(true);
    setQuizPhase('category');
  ...
  const computeQuizResult = (answers) => {
    const candidates = QUIZ_CANDIDATES[quizCategory] || [];
    const scores = {};
      ...
      chosen.forEach((val) => {
        ...
        if (opt.scores) {
          Object.keys(opt.scores).forEach((pid) => {
            if (Object.prototype.hasOwnProperty.call(scores, pid)) scores[pid] += opt.scores[pid];
          });
        }
  ...
  const quizNext = () => {
    if (quizPhase === 'category') {
      const chosen = (quizAnswers.categoria || [])[0];
      if (!chosen) return;
      setQuizCategory(chosen);
    ...
    if (quizPhase === 'questions') {
      const q = quizCurrentQuestion();
      if (!q || !(quizAnswers[q.id] || []).length) return;
      const qs = quizQuestions();
      if (quizQIndex < qs.length - 1) {
        setQuizQIndex((i) => i + 1);
      } else {
        setQuizResult(computeQuizResult(quizAnswers));
        setQuizPhase('reveal');
      }
    }
  };
  ...
  const quizBack = () => {
    if (quizPhase === 'questions') {
      if (quizQIndex > 0) setQuizQIndex((i) => i - 1);
      else { setQuizPhase('category'); setQuizCategory(null); }
      return;
    }
    if (quizPhase === 'reveal') {
      setQuizPhase('questions');
      setQuizQIndex(Math.max(0, quizQuestions().length - 1));
      return;
    }
    if (quizPhase === 'result') {
      setQuizPhase('reveal');
  ...
  const quizViewProduct = () => {
    if (!quizResult) return;
    const pid = quizResult.productId;
    const variantHint = quizResult.variantHint;
    closeQuiz();
    openProduct(pid);
    // openProduct() ya elige la variante destacada/primera por defecto; si el
    // quiz detectó un tamaño específico (ej. Queen/King del duvet), lo
    // aplicamos después para que prevalezca.
    if (variantHint) {
      const p = PRODUCTS.find((x) => x.id === pid);
      if (p && p.variants.some((v) => v.name === variantHint)) {
        setPdSelectedVariant(variantHint);
      }
    }
  };
  ...
  // Renders the quiz overlay's inner content for whichever phase it's in —
  // mirrors renderQuiz() in index.html so both versions behave the same way.
  const renderQuizBody = () => {
    if (quizPhase === 'category' || quizPhase === 'questions') {
      const q = quizCurrentQuestion();
      if (!q) return null;
      const tCopy = q[lang] || q.es;
      const isMulti = !!q.multi;
      const answered = (quizAnswers[q.id] || []).length > 0;
      return (
        <>
          <h3 className="font-serif text-2xl sm:text-3xl text-ink text-center leading-snug">{tCopy.title}</h3>
          {tCopy.subtitle && <p className="text-mist text-sm text-center mt-2">{tCopy.subtitle}</p>}
          <div className={`grid ${q.options.length > 4 ? 'sm:grid-cols-2' : 'grid-cols-1'} gap-3 mt-8`}>
            {q.options.map((o) => {
              const label = o[lang] || o.es;
              const isSelected = (quizAnswers[q.id] || []).includes(o.value);
              return (
                <button
                  key={o.value}
                  type="button"
                  onClick={() => quizSelect(o.value)}
                  className={`text-left border p-4 transition-colors flex items-center justify-between ${isSelected ? 'border-gold bg-cream' : 'border-pearl hover:border-gold'}`}
                >
                  <span className="text-ink text-sm sm:text-base">{label}</span>
                  {isMulti ? (
                    <span className={`w-5 h-5 border flex items-center justify-center text-white text-[0.65rem] flex-shrink-0 ml-3 ${isSelected ? 'bg-gold border-gold' : 'border-mist'}`}>{isSelected ? '✓' : ''}</span>
                  ) : (
                    <span className={`w-4 h-4 rounded-full border flex items-center justify-center flex-shrink-0 ml-3 ${isSelected ? 'border-gold' : 'border-mist'}`}>{isSelected && <span className="w-2 h-2 rounded-full bg-gold block" />}</span>
                  )}
                </button>
              );
            })}
          </div>
          <div className="flex items-center justify-between mt-10 pt-6 border-t border-pearl">
            <button type="button" onClick={quizBack} className="btn-outline !px-6">{t('Atrás', 'Back')}</button>
            <button
              type="button"
              onClick={quizNext}
              className={`btn-gold !px-8 ml-auto ${!answered ? 'opacity-40 pointer-events-none' : ''}`}
            >
              {t('Siguiente', 'Next')}
            </button>
          </div>
    ...
    if (quizPhase === 'reveal') {
      return (
        <div className="text-center py-6">
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#B8963E" strokeWidth="1.4" className="mx-auto mb-4">
            <path d="M9.5 9a2.5 2.5 0 1 1 3.4 2.33c-.86.34-1.4 1.2-1.4 2.17v.5" strokeLinecap="round" strokeLinejoin="round" />
            <circle cx="12" cy="17.5" r="0.9" fill="#B8963E" stroke="none" />
          </svg>
          <h3 className="font-serif text-2xl sm:text-3xl text-ink">{t('Tu resultado está listo', 'Your result is ready')}</h3>
          <p className="text-graphite text-sm mt-3 max-w-sm mx-auto leading-relaxed">
            {t('Analizamos tus respuestas junto a las especificaciones de cada producto Nuvela.', "We matched your answers against every Nuvela product's specifications.")}
          </p>
          <button type="button" onClick={() => setQuizPhase('result')} className="btn-gold mt-8 !px-10">{t('Ver mi resultado', 'See my result')}</button>
        </div>
      );
    }

    if (quizPhase === 'result') {
      const p = PRODUCTS.find((x) => x.id === (quizResult && quizResult.productId));
      if (!p) return null;
      const blurbObj = RESULT_BLURBS[p.id];
      const blurb = blurbObj ? (blurbObj[lang] || blurbObj.es) : pick(p.cardText);
      const from = priceFrom(p);
      return (
        <div className="text-center">
          <p className="eyebrow">{t('Tu producto ideal', 'Your ideal product')}</p>
          <div className="aspect-[4/3] max-w-xs mx-auto mt-4 bg-cream overflow-hidden border border-pearl img-fallback">
            <img src={p.mainImage} alt={pick(p.name)} className="w-full h-full object-cover" onError={imgFallback} />
          </div>
          <h3 className="font-serif text-2xl sm:text-3xl text-ink mt-5">{pick(p.name)}</h3>
          <p className="text-graphite text-sm mt-3 max-w-md mx-auto leading-relaxed">{blurb}</p>
          <p className="text-mist text-[0.6rem] tracking-[0.18em] uppercase mt-5">{t('Desde', 'From')}</p>
          <p className="font-price text-2xl text-gold mt-1">{from ? `Q${from.toLocaleString('en-US')}` : t('Próximamente', 'Coming soon')}</p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center mt-8">
            <button type="button" onClick={quizViewProduct} className="btn-gold !px-8">{t('Ver Producto', 'View Product')}</button>
            <button type="button" onClick={quizGoCita} className="btn-outline !px-8">{t('Agendar Cita', 'Book a Visit')}</button>
          </div>
          <button type="button" onClick={openQuiz} className="text-xs tracking-[0.18em] uppercase text-mist hover:text-gold underline underline-offset-4 mt-6">
            {t('Volver a hacer el examen', 'Retake the quiz')}
          </button>
        </div>
      );
    }
  ...
  const navItems = [
    ...
    { id: 'linea-hotelera', es: 'Línea para Hoteles', en: 'Hotel Line' },
    ...
    <div className="bg-white text-graphite font-sans">
      ...
      <header className={`sticky top-0 z-50 bg-[#0D0D0D]/95 backdrop-blur-md border-b border-white/10 transition-all ${scrolled ? 'shadow-sm' : ''}`}>
        <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          {/* Consolidado: en escritorio solo se muestran los enlaces de mayor uso/conversión; */}
          {/* el resto vive en las subpestañas "Productos" y "Más" para aligerar el menú. */}
          <nav className="hidden lg:flex items-center gap-9">
            {['home', 'producto', 'linea-hotelera', 'carrito', 'cita'].map((id) => {
              const item = navItems.find((n) => n.id === id);
              if (!item) return null;
              if (item.id === 'producto') {
                return (
                  <div key={item.id} className="relative group">
                    <a onClick={() => go(item.id)} className={`nav-link inline-flex items-center gap-1 ${page === item.id ? 'active' : ''}`}>
                      <span>{t(item.es, item.en)}</span>
                      <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="mt-0.5 opacity-70"><path d="M6 9l6 6 6-6" strokeLinecap="round" strokeLinejoin="round" /></svg>
                    </a>
                    {/* Subpestaña de productos: agrupada por categoría, con Comparar Productos al final */}
                    <div className="absolute left-1/2 -translate-x-1/2 top-full pt-4 opacity-0 invisible translate-y-1 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-200 z-50">
                      <div className="bg-[#0D0D0D] border border-white/10 min-w-[260px] max-h-[70vh] overflow-y-auto py-2 shadow-2xl">
                        <div className="pb-2 mb-1 border-b border-white/10">
                          <button
                            type="button"
                            onClick={openQuiz}
                            className="w-full text-left px-5 py-2.5 text-sm text-gold hover:text-gold-light hover:bg-white/5 transition-colors flex items-center gap-2"
                          >
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="flex-shrink-0"><path d="M9.5 9a2.5 2.5 0 1 1 3.4 2.33c-.86.34-1.4 1.2-1.4 2.17v.5" strokeLinecap="round" strokeLinejoin="round" /><circle cx="12" cy="17.5" r="0.9" fill="currentColor" stroke="none" /></svg>
                            {t('Examen de Productos', 'Product Quiz')}
                          </button>
                        </div>
                        {getCategories().map((cat) => {
                          ...
                          return (
                            <div key={cat.es}>
                              <div className="px-5 pt-3 pb-1 text-[0.62rem] tracking-[0.2em] uppercase text-mist">{pick(cat)}</div>
                              {items.map((p) => (
                                <button
                                  key={p.id}
                                  type="button"
                                  onClick={() => openProduct(p.id)}
                                  className="block w-full text-left px-5 py-2 text-sm text-cream/90 hover:text-gold hover:bg-white/5 transition-colors"
                                >
                                  {pick(p.name)}
                                </button>
                              ))}
                        ...
                        <div className="border-t border-white/10 mt-2 pt-2">
                          <a onClick={() => go('comparar')} className="block px-5 py-2.5 text-sm text-gold hover:text-gold-light hover:bg-white/5 transition-colors cursor-pointer">
                            {t('Comparar Productos', 'Compare Products')}
                          </a>
                        </div>
                      </div>
                    </div>
            ...
            <div className="relative group">
              ...
              <div className="absolute right-0 top-full pt-4 opacity-0 invisible translate-y-1 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-200 z-50">
                <div className="bg-[#0D0D0D] border border-white/10 min-w-[220px] py-2 shadow-2xl">
                  {['historia', 'tecnologia', 'entregas', 'resenas', 'faq', 'contacto'].map((id) => {
                    const item = navItems.find((n) => n.id === id);
                    if (!item) return null;
                    return (
                      <a
                        key={item.id}
                        onClick={() => go(item.id)}
                        className="block px-5 py-2.5 text-sm text-cream/90 hover:text-gold hover:bg-white/5 transition-colors cursor-pointer"
                      >
                        {t(item.es, item.en)}
                      </a>
                    );
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => go('producto')} className="hidden md:inline-flex btn-gold !py-2.5 !px-5 !text-[0.7rem]">
              {t('Comprar', 'Shop Now')}
            </button>
      ...
      {quizOpen && (
        <div className="fixed inset-0 z-[90]">
          <div onClick={closeQuiz} className="absolute inset-0 bg-ink/60" />
          <div className="relative h-full w-full flex items-center justify-center p-4 sm:p-6">
            <div className="relative bg-white border-2 border-gold w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 sm:p-10">
              <button type="button" onClick={closeQuiz} aria-label={t('Cerrar', 'Close')} className="absolute top-4 right-4 w-9 h-9 flex items-center justify-center text-ink hover:text-gold">
              ...
              <p className="eyebrow text-center">{t('Examen de Productos', 'Product Quiz')}</p>
              <div className="max-w-xs mx-auto mt-3 h-1 bg-pearl">
                <div className="h-1 bg-gold transition-all duration-300" style={{ width: `${quizProgressFraction()}%` }} />
              </div>
              <div className="mt-10">{renderQuizBody()}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {page === 'home' && (
        <main className="page-fade">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6 h-full flex flex-col justify-center">
              <div className="max-w-2xl">
                ...
                <div className="flex flex-wrap gap-4 mt-10 reveal reveal-delay-3">
                  <button onClick={() => go('producto')} className="btn-gold">{t('Ver Colchones', 'Explore Mattrresses')}</button>
          ...
          {/* TECHNOLOGY SHOWCASE */}
          <section className="py-14 md:py-32 bg-ink text-white">
          ...
          <section className="py-14 md:py-32 bg-ink text-white">
            <div className="max-w-4xl mx-auto px-6 text-center reveal">
              ...
              <div className="flex flex-wrap gap-4 justify-center mt-10">
                ...
                <button onClick={() => go('producto')} className="btn-outline-light">{t('Ver Productos', 'View Products')}</button>
              </div>
            </div>
          </section>
        </main>
      ...
      {page === 'producto' && (
        <main className="page-fade">
          ...
          {/* CATEGORÍAS + CUADRÍCULA — estilo "elige primero, luego mira el detalle" */}
          {/* (como apple.com/iphone): el cliente toca una categoría y solo entonces */}
          {/* aparece la cuadrícula de esa categoría, para no mostrar todo de golpe. */}
          <section className="pb-24 md:pb-32 bg-cream">
            <div className="max-w-6xl mx-auto px-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 sm:gap-5 mb-6">
              <button
                type="button"
                onClick={openQuiz}
                className="group relative overflow-hidden aspect-[4/3] sm:aspect-[16/11] bg-ink border border-ink text-left"
              >
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-1.5 sm:gap-2 p-3 sm:p-5 text-center">
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#B8963E" strokeWidth="1.6" className="sm:w-7 sm:h-7">
                    <path d="M9.5 9a2.5 2.5 0 1 1 3.4 2.33c-.86.34-1.4 1.2-1.4 2.17v.5" strokeLinecap="round" strokeLinejoin="round" />
                    <circle cx="12" cy="17.5" r="0.9" fill="#B8963E" stroke="none" />
                  </svg>
                  <p className="font-serif text-sm sm:text-xl text-white leading-tight">{t('Examen de Productos', 'Product Quiz')}</p>
                  <p className="text-gold text-[0.55rem] sm:text-[0.65rem] tracking-[0.18em] uppercase">{t('Encuentra el tuyo', 'Find your match')}</p>
                </div>
              </button>
              {getCategories().map((c) => {
                ...
                return (
                  <button
                    key={c.es}
                    type="button"
                    onClick={() => selectCategory(c.es)}
                    className={`group relative overflow-hidden aspect-[4/3] sm:aspect-[16/11] bg-cream border text-left ${isActive ? 'border-gold border-2' : 'border-pearl'}`}
                  >
                    {sample && (
                      <img src={sample.mainImage} alt={pick(c)} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                    )}
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/15 to-transparent" />
                    <div className="absolute inset-0 flex flex-col justify-end p-3 sm:p-5">
                      <p className="font-serif text-sm sm:text-xl text-white leading-tight">{pick(c)}</p>
                      <p className="text-white/75 text-[0.55rem] sm:text-[0.65rem] tracking-[0.18em] uppercase mt-1">
                        {count} {count === 1 ? t('modelo', 'model') : t('modelos', 'models')}
                      </p>
                    </div>
                    {isActive && (
                      <span className="absolute top-2 right-2 sm:top-3 sm:right-3 w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gold flex items-center justify-center text-white text-[0.65rem]">✓</span>
                    )}
                  </button>
                );
              })}
            ...
            <p className="text-center mb-12">
              <button
                ...
                className="text-xs tracking-[0.22em] uppercase text-mist hover:text-gold underline underline-offset-4 transition-colors"
              >
                {t('o ver todos los productos', 'or view all products')}
              </button>
            </p>
            ...
            <div ref={productsGridRef} className="max-w-7xl mx-auto px-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
                {activeCategory !== null && visibleProducts.length === 0 && (
      ...
      {page === 'linea-hotelera' && (
        <main className="page-fade">
          ...
          <section id="quote-tray-section" className="pb-12 bg-white border-b border-pearl">
                ...
                <>
                  ...
                  <div className="mt-3">
                    {quoteCart.map((entry) => {
                      ...
                      if (!line) return null;
                      const { p: prod, v, qty } = line;
                      const rowKey = `${prod.id}::${v ? v.name : ''}`;
                      return (
                        <div
                          key={`${prod.id}-${v ? v.name : ''}`}
                          id={quoteRowDomId(prod.id, v ? v.name : '')}
                          className={`flex items-center gap-4 py-4 border-b border-pearl transition-colors duration-500 ${quoteRowHighlight[rowKey] ? 'bg-cream' : ''}`}
                        >
                  ...
                  <div className="mt-6 pt-6 border-t border-pearl">
                    ...
                    <div className="grid sm:grid-cols-2 gap-4 mt-5">
                      ...
                      <div>
                        <label className="text-[0.65rem] tracking-[0.14em] uppercase text-graphite">{t('Correo *', 'Email *')}</label>
                        <input
                          type="email"
                          autoComplete="email"
                          value={quoteForm.correo}
                          onChange={(e) => updateQuoteField('correo', e.target.value)}
                          className="w-full border border-pearl px-3 py-2.5 mt-1.5 text-sm bg-white focus:border-gold outline-none"
                        />
                      </div>
                      <div>
          ...
          <section className="pb-24 md:pb-32 bg-cream">
            <div className="max-w-7xl mx-auto px-6 grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
                ...
                <>
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-8">
                    ...
                    <div className="text-right">
                      ...
                      <p className="font-price text-3xl text-gold">{formatPrice(finalCartTotal())}</p>
                      {discountAmount() > 0 && (
                        <p className="text-graphite text-sm mt-1">
                          {t('Subtotal', 'Subtotal')}: {formatPrice(cartTotal())} &nbsp;·&nbsp; {t('Descuento', 'Discount')} ({appliedDiscountCode}):{' '}
                          <span style={{ color: '#B3261E' }}>-{formatPrice(discountAmount())}</span>
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="pt-2">
                    <Eyebrow>{t('Código de Descuento', 'Discount Code')}</Eyebrow>
                    <div className="flex flex-col sm:flex-row gap-3 mt-5">
                      <input
                        type="text"
                        autoCapitalize="characters"
                        autoComplete="off"
                        placeholder={t('Ingresa tu código', 'Enter your code')}
                        value={discountInput}
                        onChange={(e) => setDiscountInput(e.target.value)}
                        onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); applyDiscountCode(); } }}
                        className="w-full sm:flex-1 border border-pearl px-3 py-2.5 text-sm bg-white focus:border-gold outline-none uppercase"
                      />
                      <button type="button" onClick={applyDiscountCode} className="btn-outline !py-2.5 !px-6 whitespace-nowrap">
                        {t('Aplicar', 'Apply')}
                      </button>
                    </div>
                    {appliedDiscountCode && (
                      DISCOUNT_CODES[appliedDiscountCode] ? (
                        cartHasCategory(DISCOUNT_CODES[appliedDiscountCode].requiresCategory.es) ? (
                          <p className="text-sm mt-3 font-medium" style={{ color: '#53575A' }}>
                            {t(
                              `✓ Código ${appliedDiscountCode} aplicado: -${formatPrice(DISCOUNT_CODES[appliedDiscountCode].amount)}`,
                              `✓ Code ${appliedDiscountCode} applied: -${formatPrice(DISCOUNT_CODES[appliedDiscountCode].amount)}`
                            )}
                          </p>
                        ) : (
                          <p className="text-sm mt-3 font-medium" style={{ color: '#B3261E' }}>
                            {t('Este código solo es válido al agregar un colchón a tu pedido.', 'This code is only valid when a mattress is added to your order.')}
                          </p>
                        )
                      ) : (
                        <p className="text-sm mt-3 font-medium" style={{ color: '#B3261E' }}>
                          {t('Código no válido.', 'Invalid code.')}
                        </p>
                      )
                    )}
                  </div>
      ...
      {page === 'producto-detalle' && selectedProduct && (
        <main className="page-fade">
          <section className="bg-cream pt-28 pb-12 md:pb-20">
            <div className="max-w-7xl mx-auto px-6">
              <button onClick={() => go('producto')} className="link-gold mb-6 md:mb-10">{t('← Volver a Productos', '← Back to Products')}</button>

              {/* Botón principal centrado: hace lo mismo que "Comparar Productos" del menú */}
              <div className="flex justify-center mb-6 md:mb-10">
                <button onClick={() => go('comparar')} className="btn-gold !px-8">{t('Comparar Productos', 'Compare Products')}</button>
              </div>
              ...
              <div className="grid lg:grid-cols-2 gap-8 md:gap-16 items-start">
                ...
                <div className="reveal reveal-delay-1">
                  ...
                  <div className="flex flex-wrap gap-4 mt-6">
                    <button onClick={() => go('contacto')} className="btn-outline w-full sm:w-auto">{t('Hablar con Concierge', 'Contact Concierge')}</button>
                  </div>
                </div>
              </div>
            </div>
          ...
          {selectedProduct.benefits && selectedProduct.benefits.length > 0 && (
            <section className="py-14 md:py-24 bg-white">
              <div className="max-w-7xl mx-auto px-6">
                <div className={`mobile-collapse-section reveal ${pdBenefitsOpen ? 'open' : ''}`}>
                  <button
                    type="button"
                    onClick={() => setPdBenefitsOpen((v) => !v)}
                    aria-expanded={pdBenefitsOpen}
                    className="mobile-collapse-toggle w-full flex flex-col items-center text-center max-w-2xl mx-auto cursor-pointer md:cursor-default"
                  >
          ...
          <section className="py-14 md:py-24 bg-cream">
            <div className="max-w-5xl mx-auto px-6">
              <div className={`mobile-collapse-section reveal ${pdSpecsOpen ? 'open' : ''}`}>
                <button
                  type="button"
                  onClick={() => setPdSpecsOpen((v) => !v)}
                  aria-expanded={pdSpecsOpen}
                  className="mobile-collapse-toggle w-full flex flex-col items-center text-center cursor-pointer md:cursor-default"
                >
        ...
        <main className="page-fade">
          ...
          {PRODUCTS.map((p, pi) => (
            <section
              ...
              style={{ backgroundImage: `url('${PRODUCT_TEXTURES[p.id] || 'images/colchas.jpg'}')` }}
            >
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {p.variants.map((v, i) => (
                    ...
                    >
                      ...
                      <button onClick={() => go('carrito')} className="btn-outline mt-3 w-full">
                        {t('Ver mi Carrito', 'View My Cart')}
                      </button>
                    </div>
                  ))}
                </div>
        ...
        <main className="page-fade">
          ...
          <section className="pb-24 md:pb-32 bg-cream border-t border-pearl">
            <div className="max-w-4xl mx-auto px-6 pt-16">
              ...
              <div className="text-center">
                {CITA_SHOWROOM_CALENDAR_URL ? (
                  <a href={CITA_SHOWROOM_CALENDAR_URL} target="_blank" rel="noopener noreferrer" className="btn-gold mt-8 !py-3.5 !px-10 inline-block">{t('Ver horarios y agendar', 'View times and book')}</a>
                ) : (
                  <>
                    <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" rel="noopener noreferrer" className="btn-gold mt-8 !py-3.5 !px-10 inline-block">{t('Escríbenos por WhatsApp', 'Message us on WhatsApp')}</a>
            ...
            <div className="max-w-4xl mx-auto px-6 pt-16">
              ...
              <div className="mt-8 grid gap-8">
                {SUCURSALES.map((s) => {
                  ...
                  return (
                    <div key={s.name.es} className="border border-pearl bg-cream overflow-hidden sm:flex">
                      <div className="sm:w-1/2 aspect-[4/3] sm:aspect-auto bg-pearl">
                        <iframe
                          src={embedSrc}
                      ...
                      <div className="sm:w-1/2 p-6 sm:p-8 flex flex-col justify-center">
                        <p className="font-serif text-xl text-ink">{pick(s.name)}</p>
                        <p className="text-graphite text-sm mt-2 leading-relaxed">{pick(s.detail)}</p>
                        <a href={mapsLink} target="_blank" rel="noopener noreferrer" className="btn-gold mt-5 !px-6 self-start">{t('Cómo llegar', 'Get Directions')}</a>
                      </div>
                    </div>
                  );
                })}
        ...
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div>
            ...
            <ul className="mt-6 space-y-3 text-white/70 text-sm">
              {['historia', 'producto', 'linea-hotelera', 'comparar', 'tecnologia', 'carrito'].map((p) => {
...
function testimonialsData(t) {
  return [
    { quote: t('«La primera noche fue el sueño más profundo que he tenido en años. Nuvela no es un colchón — es un ritual.»', "“The first night was the deepest sleep I've had in years. Nuvela isn't a mattress — it's a ritual.”"), name: 'Paulina E.', meta: t('King · Ciudad de Guatemala', 'King · Guatemala City') },
  ];
}
